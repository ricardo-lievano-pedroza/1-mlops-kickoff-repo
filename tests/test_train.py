...
def test_train_model_classification_fail():
    ...
    with pytest.raises(ValueError, match="Training failed: problem_type not supported"):
        train_model(X_train, y_train_binary, preprocessor, problem_type='classification')
...
def test_train_model_mismatched_X_y_lengths():
    """Test that training fails when X_train and y_train have different lengths."""
...
def test_train_model_predictions_shape(sample_data, preprocessor):
...
def test_train_model_none_preprocessor(sample_data):
    """Test that training fails with None preprocessor."""
    X_train, y_train = sample_data
    with pytest.raises((ValueError, TypeError)):
